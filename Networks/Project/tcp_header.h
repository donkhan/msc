...
struct tcp_header* get_sample_tcp_header(){
	struct tcp_header* tcp_header = (struct tcp_header *) malloc(sizeof(struct tcp_header));
...
char* encapsulate_tcp_header(struct tcp_header* tcp_header){
	...
	tcp_packet = strcat(tcp_packet,int_to_binary(tcp_header->tcp_checksum,16));
	tcp_packet = strcat(tcp_packet,int_to_binary(tcp_header->urgent_pointer,16));
	for(int i = 0;i<tcp_header->hlen;i++){
...
void print_tcp_header(struct tcp_header* tcp_header){
	...
	printf("\nFlags Urg %d Ack %d Psh %d Rst %d Syn %d Fin %d",tcp_header->urg,tcp_header->ack,
